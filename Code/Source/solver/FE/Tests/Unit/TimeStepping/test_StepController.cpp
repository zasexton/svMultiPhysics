...
class AlwaysThrowLinearSolver final : public svmp::FE::backends::LinearSolver {
public:
    explicit AlwaysThrowLinearSolver(svmp::FE::backends::LinearSolver& inner)
        : inner_(inner)
    {
    }
    ...
    {
        ++solve_calls_;
        throw svmp::FE::FEException("time-loop-adaptive-exception-sentinel");
    }

    [[nodiscard]] int solveCalls() const noexcept { return solve_calls_; }
...
class RetryOnceThenStopController final : public svmp::FE::timestepping::StepController {
public:
               ...
               svmp::FE::timestepping::StepRejectReason) override
    {
        svmp::FE::timestepping::StepDecision d;
        d.accept = false;
        d.retry = (info.attempt_index == 0);
        d.next_dt = d.retry ? 0.5 * info.dt : 0.0;
        d.message = d.retry ? "retrying after exception" : "adaptive retry budget exhausted";
        return d;
    }
};
...
TEST(TimeLoopAdaptiveStep, FinalRetryReportPreservesUnderlyingException)
{
#if !defined(FE_HAS_EIGEN) || !FE_HAS_EIGEN
    GTEST_SKIP() << "TimeStepping tests require the Eigen backend (enable FE_ENABLE_EIGEN)";
#endif
    ...
    const auto v = svmp::FE::forms::FormExpr::testFunction(*space, "v");
    const auto form =
        (svmp::FE::forms::dt(u) * v + (u * v) * static_cast<Real>(1.0)).dx();
    ...
    AlwaysThrowLinearSolver linear(*inner);

    auto history = svmp::FE::timestepping::TimeHistory::allocate(
        *factory, sys.dofHandler().getNumDofs());
    const std::vector<Real> u0 = {1.0, -0.5, 0.25, 2.0};
    ts_test::setVectorByDof(history.uPrev(), u0);
    ts_test::setVectorByDof(history.uPrev2(), u0);
    history.resetCurrentToPrevious();
    ...
    opts.newton.abs_tolerance = 1e-12;
    opts.newton.rel_tolerance = 0.0;
    opts.step_controller = std::make_shared<RetryOnceThenStopController>();

    svmp::FE::timestepping::TimeLoop loop(opts);
    const auto rep = loop.run(transient, *factory, linear, history);

    EXPECT_FALSE(rep.success);
    EXPECT_EQ(linear.solveCalls(), 2);
    EXPECT_NE(rep.message.find("adaptive retry budget exhausted"), std::string::npos)
        << rep.message;
    EXPECT_NE(rep.message.find("exception:"), std::string::npos) << rep.message;
    EXPECT_NE(rep.message.find("time-loop-adaptive-exception-sentinel"),
              std::string::npos)
        << rep.message;
    EXPECT_NEAR(history.time(), 0.0, 1e-15);
}
...
TEST(TimeLoopCallbacks, FixedStepDoesNotAcceptNonconvergedStep)
{
#if !defined(FE_HAS_EIGEN) || !FE_HAS_EIGEN
    GTEST_SKIP() << "TimeStepping tests require the Eigen backend (enable FE_ENABLE_EIGEN)";
#endif
    ...
    int nonlinear_done_calls = 0;
    int step_accepted_calls = 0;
    bool last_nonlinear_converged = true;

    svmp::FE::timestepping::TimeLoopCallbacks callbacks;
    callbacks.on_nonlinear_done = [&nonlinear_done_calls, &last_nonlinear_converged](
    ...
    svmp::FE::timestepping::TimeLoop loop(opts);
    EXPECT_THROW((void)loop.run(transient, *factory, *linear, history, callbacks), svmp::FE::FEException);
    EXPECT_EQ(nonlinear_done_calls, 1);
    EXPECT_FALSE(last_nonlinear_converged);
    EXPECT_EQ(step_accepted_calls, 0);
    EXPECT_NEAR(history.time(), 0.0, 1e-15);
}
...
TEST(TimeLoopEdgeCases, AbsorbsSmallTerminalRemainderIntoPreviousStep)
{
#if !defined(FE_HAS_EIGEN) || !FE_HAS_EIGEN
    GTEST_SKIP() << "TimeStepping tests require the Eigen backend (enable FE_ENABLE_EIGEN)";
#endif
    ...
    opts.max_steps = 4;
    opts.adjust_last_step = true;
    opts.last_step_absorb_fraction = 0.03;
    opts.scheme = svmp::FE::timestepping::SchemeKind::BackwardEuler;
    opts.newton.residual_op = "op";
    opts.newton.jacobian_op = "op";
    opts.newton.max_iterations = 8;
    ...
    std::vector<double> step_start_dts;
    svmp::FE::timestepping::TimeLoopCallbacks callbacks;
    callbacks.on_step_start = [&step_start_dts](const svmp::FE::timestepping::TimeHistory& h) {
        step_start_dts.push_back(h.dt());
    };

    svmp::FE::timestepping::TimeLoop loop(opts);
    const auto rep = loop.run(transient, *factory, *linear, history, callbacks);
    EXPECT_TRUE(rep.success) << rep.message;
    EXPECT_EQ(rep.steps_taken, 2);
    ASSERT_EQ(step_start_dts.size(), 2u);
    EXPECT_NEAR(step_start_dts[0], 0.495, 1e-12);
    EXPECT_NEAR(step_start_dts[1], 0.505, 1e-12);
    EXPECT_NEAR(rep.final_time, 1.0, 1e-12);
    EXPECT_NEAR(history.time(), 1.0, 1e-12);
    EXPECT_NEAR(history.dtPrev(), 0.505, 1e-12);
}

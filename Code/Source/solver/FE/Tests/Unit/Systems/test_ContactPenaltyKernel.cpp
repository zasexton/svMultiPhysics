/**
 * @file test_ContactPenaltyKernel.cpp
 * @brief Unit tests for global, search-driven penalty contact kernel.
 */
...
using svmp::FE::systems::PenaltyContactConfig;
using svmp::FE::systems::PenaltyPointContactKernel;
...
TEST(ContactPenaltyKernel, AssemblesOppositeForces)
{
    ...
    PenaltyContactConfig cfg;
    cfg.field = u;
    cfg.slave_marker = slave_marker;
    cfg.master_marker = master_marker;
    cfg.search_radius = 0.5;
    cfg.activation_distance = 0.2;
    cfg.penalty = 10.0;

    sys.addGlobalKernel("contact", std::make_shared<PenaltyPointContactKernel>(cfg));
    sys.setup();
    ...
    // Expected magnitude per vertex pair: k * (h - d) = 10 * (0.2 - 0.1) = 1.
    const Real fx = -1.0;
...
TEST(ContactPenaltyKernel, AssemblesSymmetricJacobian)
{
    const int slave_marker = 10;
    const int master_marker = 20;
    const double gap = 0.1;
    ...
    // For r = (-gap, 0), d = gap:
    // J = k*(h/d - 1) I - (k*h/d^3) (r ⊗ r)
    // => J_xx = -10, J_yy = 10 for k=10, h=0.2, d=0.1.
    const Real tol = 1e-10;
    EXPECT_NEAR(jac(v1x, v1x), -10.0, tol);
    EXPECT_NEAR(jac(v1x, v4x), 10.0, tol);
    EXPECT_NEAR(jac(v4x, v1x), 10.0, tol);
    EXPECT_NEAR(jac(v4x, v4x), -10.0, tol);

    EXPECT_NEAR(jac(v1y, v1y), 10.0, tol);
    EXPECT_NEAR(jac(v1y, v4y), -10.0, tol);
    EXPECT_NEAR(jac(v4y, v1y), -10.0, tol);
    EXPECT_NEAR(jac(v4y, v4y), 10.0, tol);
}

...
TEST(VectorSpaceTraces, HCurlHybridOrder3TangentialTraceIsProjection) {
    HCurlSpace space(ElementType::Pyramid5, 3);
    const auto ndofs = space.dofs_per_element();
    ASSERT_GT(ndofs, 0u);

    std::vector<Real> dof_values(ndofs);
    for (std::size_t i = 0; i < ndofs; ++i) {
        dof_values[i] = Real(0.01) * Real(i + 1);
    }

    std::vector<HCurlSpace::Vec3> points = {
        as_vec3(Real(0.0), Real(0.0), Real(0.25)),
        as_vec3(Real(0.1), Real(-0.2), Real(0.4)),
        as_vec3(Real(-0.15), Real(0.1), Real(0.5)),
    };
    ...
    for (std::size_t i = 0; i < points.size(); ++i) {
        ...
        EXPECT_NEAR(tangential[i][0], expected[0], 1e-10);
        EXPECT_NEAR(tangential[i][1], expected[1], 1e-10);
        EXPECT_NEAR(tangential[i][2], expected[2], 1e-10);
        EXPECT_NEAR(tangential[i].dot(normal), 0.0, 1e-10);
    }
}

TEST(VectorSpaceTraces, HDivHybridOrder3NormalTraceMatchesDotProduct) {
    HDivSpace space(ElementType::Wedge6, 3);
    const auto ndofs = space.dofs_per_element();
    ASSERT_GT(ndofs, 0u);

    std::vector<Real> dof_values(ndofs);
    for (std::size_t i = 0; i < ndofs; ++i) {
        dof_values[i] = Real(-0.015) * Real(i + 1);
    }

    std::vector<HDivSpace::Vec3> points = {
        as_vec3(Real(0.2), Real(0.1), Real(-0.5)),
        as_vec3(Real(0.3), Real(0.2), Real(0.0)),
        as_vec3(Real(0.1), Real(0.1), Real(0.5)),
    };

    const HDivSpace::Vec3 normal = as_vec3(Real(0), Real(0), Real(1));
    auto normal_trace = space.normal_trace(dof_values, points, normal);
    ...
    for (std::size_t i = 0; i < points.size(); ++i) {
        const auto v = space.evaluate(points[i], dof_values);
        const Real expected = v.dot(normal);
        EXPECT_NEAR(normal_trace[i], expected, 1e-10);
    }
}

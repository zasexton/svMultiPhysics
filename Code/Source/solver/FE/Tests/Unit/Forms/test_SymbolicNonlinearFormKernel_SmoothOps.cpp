...
/**
 * @file test_SymbolicNonlinearFormKernel_SmoothOps.cpp
 * @brief Numerical Jacobian verification for smooth approximation operators (symbolic tangents).
 */
...
TEST(SymbolicSmoothOpsJacobianTest, SmoothHeavisideCoefficientMatchesCentralDifferences)
{
    ...
    // Diffusion-like term with smooth Heaviside coefficient:
    //   ∫ H_eps(u) * ∇u · ∇v dx
    const auto residual = inner(smoothHeaviside(u, eps) * grad(u), grad(v)).dx();

    const std::vector<Real> U = {-0.2, 0.1, 0.25, -0.05};
    expectSymbolicJacobianMatchesCentralDifferences(mesh, space, residual, U, /*eps=*/2e-6, /*tol=*/5e-7);
}

TEST(SymbolicSmoothOpsJacobianTest, SmoothMinAndMaxMatchCentralDifferences)
{
    SingleTetraMeshAccess mesh;
    spaces::H1Space space(ElementType::Tetra4, /*order=*/1);
    ...
    const auto u = TrialFunction(space, "u");
    const auto v = TestFunction(space, "v");
    const auto eps = FormExpr::constant(Real(0.15));
    const auto c = FormExpr::constant(Real(0.05));

    // Combine both operators so we hit both code paths in one assembly:
    //   ∫ (min_eps(u,c) + max_eps(u,c)) v dx
    const auto residual = ((smoothMin(u, c, eps) + smoothMax(u, c, eps)) * v).dx();

    const std::vector<Real> U = {0.02, 0.12, -0.08, 0.04};
    expectSymbolicJacobianMatchesCentralDifferences(mesh, space, residual, U, /*eps=*/2e-6, /*tol=*/5e-7);
}

TEST(SymbolicSmoothOpsJacobianTest, SmoothSignMatchesCentralDifferences)
{
    SingleTetraMeshAccess mesh;
    spaces::H1Space space(ElementType::Tetra4, /*order=*/1);
    ...
    const auto residual = (smoothSign(u, eps) * v).dx();

    const std::vector<Real> U = {-0.05, 0.02, 0.08, -0.01};
    expectSymbolicJacobianMatchesCentralDifferences(mesh, space, residual, U, /*eps=*/2e-6, /*tol=*/5e-7);
}

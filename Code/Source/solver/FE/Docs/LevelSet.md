...
Physics modules remain responsible for user input, equation-specific defaults,
and formulation-specific residuals. They translate those choices into
`FE::level_set` options and call the FE services directly.
...
The level-set equation registration in Physics is an input adapter. It preserves
...
the Physics input adapter for backward compatibility.
...
2. Parse module-specific input in Physics and translate it into the matching
   `FE::level_set` option struct.

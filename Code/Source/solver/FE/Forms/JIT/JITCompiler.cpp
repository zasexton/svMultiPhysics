...
JITCompileResult JITCompiler::Impl::compileFusedFormIR(const FormIR& tangent_ir,
                                                        const FormIR& residual_ir,
                                                        const ValidationOptions& validation)
{
    JITCompileResult out;
    ...
    // Validate both IRs
    const auto vr_tan = canCompile(tangent_ir, validation);
    if (!vr_tan.ok) {
        out.message = "JITCompiler(fused): tangent IR not JIT-compatible";
        out.cacheable = false;
        return out;
    }
    const auto vr_res = canCompile(residual_ir, validation);
    if (!vr_res.ok) {
        out.message = "JITCompiler(fused): residual IR not JIT-compatible";
        out.cacheable = false;
        return out;
    }

    // Collect Cell-domain term indices from both IRs
    std::vector<std::size_t> tangent_cell_indices;
    for (std::size_t i = 0; i < tangent_ir.terms().size(); ++i) {
        if (tangent_ir.terms()[i].domain == IntegralDomain::Cell) {
            tangent_cell_indices.push_back(i);
        }
    }
    std::vector<std::size_t> residual_cell_indices;
    for (std::size_t i = 0; i < residual_ir.terms().size(); ++i) {
        if (residual_ir.terms()[i].domain == IntegralDomain::Cell) {
            residual_cell_indices.push_back(i);
        }
    }

    if (tangent_cell_indices.empty() || residual_cell_indices.empty()) {
        out.message = "JITCompiler(fused): no cell terms to fuse";
        return out;
    }

    // Build a fused cache key mixing both IR hashes with a discriminator.
    std::uint64_t combined_hash = 0xF05ED'0000'0000ULL; // "FOSED" discriminator
    const int preferred_vector_width = preferredVectorWidthFromCpuFeatures(cpu_features);

    auto hashTermsFromIR = [&](const FormIR& ir, const std::vector<std::size_t>& indices) {
    ...
    hashTermsFromIR(tangent_ir, tangent_cell_indices);
    hashMix(combined_hash, 0xDEAD'BEEF'CAFE'0ULL); // separator between tangent/residual
    hashTermsFromIR(residual_ir, residual_cell_indices);

    // Mix in compiler config
    const auto test_sig_hash = hashSpaceSig(tangent_ir.testSpace());
    const auto trial_sig_hash = hashSpaceSig(tangent_ir.trialSpace());
    std::uint64_t cache_key = kFNVOffset;
    hashMix(cache_key, static_cast<std::uint64_t>(assembly::jit::kKernelArgsABIVersionV6));
    hashMix(cache_key, combined_hash);
    hashMix(cache_key, test_sig_hash.h);
    ...
    try {
        ...
        if (addr == 0) {
            LLVMGen gen(options);
            const auto r = gen.compileAndAddFusedKernel(*engine,
                                                         tangent_ir,
                                                         tangent_cell_indices,
                                                         residual_ir,
                                                         ...
                                                         addr);
            if (!r.ok) {
                out.message = "JITCompiler(fused): " + r.message;
                out.cacheable = false;
                return out;
            }
        }
        ...
        out.message = std::string("JITCompiler(fused): LLVM compilation failed: ") + e.what();
        out.cacheable = false;
        return out;
    }
...
JITCompileResult JITCompiler::compileFused(const FormIR& tangent_ir,
                                            const FormIR& residual_ir,
                                            const ValidationOptions& validation)
{
    JITCompileResult out;

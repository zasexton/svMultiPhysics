...
#ifndef SVMP_FE_BASIS_DETAIL_REFERENCEDERIVATIVEJET_H
#define SVMP_FE_BASIS_DETAIL_REFERENCEDERIVATIVEJET_H
...
inline Jet3 constant_jet(Real value) {
    Jet3 jet;
    jet.value = value;
    return jet;
}

inline Jet3 variable_jet(int axis, Real value) {
    Jet3 jet = constant_jet(value);
    jet.gradient[static_cast<std::size_t>(axis)] = Real(1);
    return jet;
}

inline Jet3 operator+(const Jet3& lhs, const Jet3& rhs) {
    Jet3 out;
...
inline Jet3 operator-(const Jet3& jet) {
    Jet3 out;
    out.value = -jet.value;
    for (int i = 0; i < 3; ++i) {
        const std::size_t si = static_cast<std::size_t>(i);
...
inline Jet3 operator*(const Jet3& jet, Real scalar) {
    Jet3 out;
...
inline Jet3 operator*(Real scalar, const Jet3& jet) {
    return jet * scalar;
}
...
inline Jet3 compose_univariate(const Jet3& jet,
                               Real value,
...
inline Jet3 reciprocal(const Jet3& jet) {
...
inline Jet3 pow_int(const Jet3& jet, int exponent) {
    if (exponent <= 0) {
...
#endif // SVMP_FE_BASIS_DETAIL_REFERENCEDERIVATIVEJET_H

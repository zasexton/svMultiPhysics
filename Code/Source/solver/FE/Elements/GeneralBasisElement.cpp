...
GeneralBasisElement::GeneralBasisElement(std::shared_ptr<basis::BasisFunction> basis,
    ...
    if (!basis_) {
        throw FEException("GeneralBasisElement requires a valid basis pointer",
                          __FILE__, __LINE__, __func__, FEStatus::InvalidArgument);
    }
    if (!quad_) {
        throw FEException("GeneralBasisElement requires a valid quadrature rule",
                          __FILE__, __LINE__, __func__, FEStatus::InvalidArgument);
    }
    ...
    if (basis_->dimension() != quad_->dimension()) {
        throw FEException(
            "GeneralBasisElement: basis dimension does not match quadrature dimension",
            __FILE__, __LINE__, __func__, FEStatus::InvalidArgument);
    }
    ...
    if (to_mesh_family(basis_->element_type()) != quad_->cell_family()) {
        throw FEException(
            "GeneralBasisElement: basis element type does not match quadrature cell family",
            __FILE__, __LINE__, __func__, FEStatus::InvalidArgument);
    }
    ...
    if (dimension_ <= 0) {
        throw FEException("GeneralBasisElement: basis reports non-positive dimension",
    ...
    if (vector_basis) {
        if (field_type != FieldType::Vector) {
            throw FEException(
                "GeneralBasisElement: vector-valued basis requires FieldType::Vector",
                __FILE__, __LINE__, __func__, FEStatus::InvalidArgument);
        }
        if (continuity != Continuity::H_div && continuity != Continuity::H_curl) {
            throw FEException(
                "GeneralBasisElement: vector-valued basis requires Continuity::H_div or Continuity::H_curl",
                __FILE__, __LINE__, __func__, FEStatus::InvalidArgument);
        }
    } else {
        if (field_type != FieldType::Scalar) {
            throw FEException(
                "GeneralBasisElement: scalar basis currently supports FieldType::Scalar only (use Mixed/Product spaces for multi-component fields)",
                __FILE__, __LINE__, __func__, FEStatus::NotImplemented);
        }
        if (continuity == Continuity::H_div || continuity == Continuity::H_curl) {
            throw FEException(
                "GeneralBasisElement: scalar basis cannot be tagged as H(div)/H(curl) continuity",
                __FILE__, __LINE__, __func__, FEStatus::InvalidArgument);
        }
    }

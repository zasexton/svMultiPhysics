...
public:
  ...
  StackTraceFrame(std::string symbol, std::string module, std::string file,
      ...
      : symbol_(std::move(symbol)), module_(std::move(module)),
        file_(std::move(file)), line_(line), address_(address) {}
...
public:
  ...
  const std::vector<StackTraceFrame> &frames() const noexcept {
    return frames_;
  }
...
public:
  ...
  void set_source_location(const char *file, int line, const char *function) {
    ...
    line_ = line;
    function_ = (function == nullptr) ? std::string() : std::string(function);
  }
...
public:
  CoreException(const std::string &message,
                StatusCode status = StatusCode::Unknown, const char *file = "",
                int line = 0, const char *function = "")
 ...
 * `explicit Name(const std::string& message)` constructor that records @p
 * Status. Use it for exceptions that carry only a message; write the class by
 * hand when it needs extra structured context (members and accessors). The
 * source location is stamped by raise(), so no file/line/function constructor
 * is needed.
 *
...
#define SVMP_DEFINE_EXCEPTION(Name, Base, Status)                              \
  ...
  public:                                                                      \
    explicit Name(const std::string &message) : Base(message, (Status)) {}     \
  }
...
///
/// @details An exception raising the status code InternalError, used for
/// unexpected conditions that indicate a bug in the code.
...
/**
 * @brief A diagnostic message bundled with the source location where it was
 * written.
 *
 * @details The core helpers (raise(), check(), throw_if(), check_not_null())
 * take a Diagnostic in place of an explicit source location. Its
 * file/line/function arguments default to the compiler builtins
 * __builtin_FILE()/__builtin_LINE()/
 * __builtin_FUNCTION(), which capture the caller's location, so a string
 * literal or std::string passed at the call site is implicitly wrapped into a
 * Diagnostic that records exactly where the call appears -- callers pass no
 * explicit location:
 * @code
...
public:
   ...
   * @param function Function; defaults to the caller's via
   * __builtin_FUNCTION().
   */
  Diagnostic(const char *message, const char *file = __builtin_FILE(),
   ...
   * __builtin_FUNCTION().
   */
  Diagnostic(std::string message, const char *file = __builtin_FILE(),
             int line = __builtin_LINE(),
             const char *function = __builtin_FUNCTION())
 ...
 *
 * @details @p diagnostic carries the message and the source location captured
 * at the call site; @p args are forwarded to the exception constructor after
 * the message. The location is recorded via
 * ExceptionBase::set_source_location(), so exception types never need a
 * file/line/function constructor -- a `(message)` (plus any structured-context)
 * constructor is enough.
 */
template <class ExceptionT, class... Args>
[[noreturn]] void raise(Diagnostic diagnostic, Args &&...args) {
  static_assert(
      std::is_base_of_v<ExceptionBase, ExceptionT>,
 ...
 *
 * @details @p ExceptionT defaults to IndexOutOfRangeException; supply a
 * different type only when a subsystem needs its own exception. The bounds
 * message and the source location are generated automatically.
 */
template <class ExceptionT = IndexOutOfRangeException, class IndexT,
          class SizeT>
void check_index(IndexT index, SizeT size, const char *file = __builtin_FILE(),
                 int line = __builtin_LINE(),
                 const char *function = __builtin_FUNCTION()) {
  const long long index_value = static_cast<long long>(index);
  const long long size_value = static_cast<long long>(size);
  check<ExceptionT>(index_value >= 0 && index_value < size_value,
                    Diagnostic("Index " + std::to_string(index_value) +
                                   " out of bounds [0, " +
 ...
 *
 * @details @p ExceptionT defaults to NotImplementedException, so most call
 * sites pass only a message:
 * @code

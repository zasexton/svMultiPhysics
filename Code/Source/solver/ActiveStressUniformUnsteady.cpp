...
void ActiveStressUniformUnsteady::read_model_specific_parameters(
...
void ActiveStressUniformUnsteady::distribute_model_specific_parameters(
    const CmMod &cm_mod, const cmType &cm) {
  cm.bcast(cm_mod, &ramp);
  cm.bcast(cm_mod, temporal_values_file_path);
}

double ActiveStressUniformUnsteady::compute_active_tension_local(
    const Vector<double> &state, const double fiber_stretch) const {
  return fourier_interpolation.value(time)[0];
}

REGISTER_ACTIVE_STRESS_MODEL("UniformUnsteady", ActiveStressUniformUnsteady);
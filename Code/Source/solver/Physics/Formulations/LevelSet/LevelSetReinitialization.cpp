...
using FE::interfaces::CutInterfaceFragment;
using FE::interfaces::CutInterfaceFragmentKind;
...
[[nodiscard]] std::array<FE::Real, 3> add(const std::array<FE::Real, 3>& a,
                                          const std::array<FE::Real, 3>& b) noexcept
{
    return {{a[0] + b[0], a[1] + b[1], a[2] + b[2]}};
}

[[nodiscard]] std::array<FE::Real, 3> scale(const std::array<FE::Real, 3>& a,
                                            FE::Real s) noexcept
{
...
[[nodiscard]] FE::Real distance(const std::array<FE::Real, 3>& a,
                                const std::array<FE::Real, 3>& b) noexcept
{
    return norm(sub(a, b));
}

[[nodiscard]] FE::Real pointSegmentDistance(const std::array<FE::Real, 3>& p,
                                            const std::array<FE::Real, 3>& a,
                                            const std::array<FE::Real, 3>& b) noexcept
{
    const auto ab = sub(b, a);
    const FE::Real denom = dot(ab, ab);
    if (denom <= FE::Real{0.0}) {
        return distance(p, a);
    }
    const FE::Real t =
        std::clamp(dot(sub(p, a), ab) / denom, FE::Real{0.0}, FE::Real{1.0});
    return distance(p, add(a, scale(ab, t)));
}

[[nodiscard]] FE::Real pointTriangleDistance(const std::array<FE::Real, 3>& p,
                                             const std::array<FE::Real, 3>& a,
                                             ...
                                             const std::array<FE::Real, 3>& c) noexcept
{
    ...
    const FE::Real d1 = dot(ab, ap);
    const FE::Real d2 = dot(ac, ap);
    if (d1 <= FE::Real{0.0} && d2 <= FE::Real{0.0}) {
        return distance(p, a);
    }

    const auto bp = sub(p, b);
    const FE::Real d3 = dot(ab, bp);
    const FE::Real d4 = dot(ac, bp);
    if (d3 >= FE::Real{0.0} && d4 <= d3) {
        return distance(p, b);
    }

    const FE::Real vc = d1 * d4 - d3 * d2;
    if (vc <= FE::Real{0.0} && d1 >= FE::Real{0.0} && d3 <= FE::Real{0.0}) {
        const FE::Real v = d1 / (d1 - d3);
        return distance(p, add(a, scale(ab, v)));
    }

    const auto cp = sub(p, c);
    const FE::Real d5 = dot(ab, cp);
    const FE::Real d6 = dot(ac, cp);
    if (d6 >= FE::Real{0.0} && d5 <= d6) {
        return distance(p, c);
    }

    const FE::Real vb = d5 * d2 - d1 * d6;
    if (vb <= FE::Real{0.0} && d2 >= FE::Real{0.0} && d6 <= FE::Real{0.0}) {
        const FE::Real w = d2 / (d2 - d6);
        return distance(p, add(a, scale(ac, w)));
    }

    const FE::Real va = d3 * d6 - d5 * d4;
    if (va <= FE::Real{0.0} && (d4 - d3) >= FE::Real{0.0} &&
        (d5 - d6) >= FE::Real{0.0}) {
        const FE::Real w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
        return distance(p, add(b, scale(sub(c, b), w)));
    }

    const FE::Real denom = FE::Real{1.0} / (va + vb + vc);
    const FE::Real v = vb * denom;
    const FE::Real w = vc * denom;
    const auto closest = add(add(a, scale(ab, v)), scale(ac, w));
    return distance(p, closest);
}

[[nodiscard]] FE::Real pointPrimitiveDistance(const std::array<FE::Real, 3>& point,
                                              const SurfacePrimitive& primitive) noexcept
{
    ...
    if (primitive.kind == CutInterfaceFragmentKind::Polygon &&
        ...
        FE::Real best = std::numeric_limits<FE::Real>::infinity();
...
{
    switch (type) {
    case FE::ElementType::Line2:
    case FE::ElementType::Line3:
        return 2u;
...
void setCoefficientAtVertex(const FE::dofs::EntityDofMap& entity_map,
                            FE::GlobalIndex vertex,
                            FE::Real value,
...
[[nodiscard]] FE::Real nearestDistanceToInterface(
    ...
    std::vector<FE::Real>& repaired_coefficients)
{
    ...
    for (FE::GlobalIndex vertex = 0; vertex < mesh.numVertices(); ++vertex) {
        ...
        const FE::Real d = nearestDistanceToInterface(x, primitives);
        if (!std::isfinite(d)) {
            throw std::runtime_error(
                "level-set signed-distance repair produced a non-finite distance");
        }
        FE::Real repaired = 0.0;

...
[[nodiscard]] FE::FieldId resolveLevelSetField(
    ...
    FE::GlobalIndex cell_id)
{
    ...
    if (mesh.dimension() == 2) {
        cut_result = FE::interfaces::cutLinearLevelSetCell2D(domain.request(), input);
    } else if (mesh.dimension() == 3) {
        cut_result = FE::interfaces::cutLinearLevelSetCell3D(domain.request(), input);
    } else {
        throw std::invalid_argument(
    ...
    std::span<const FE::Real> solution)
{
    ...
    const auto marker_source = FE::interfaces::LevelSetInterfaceSource::fromField(field);

...
**Progress, 2026-09-05 (UTC), driver characterization:** Before changing production ownership, twelve focused cases now retain 52 complete version-4 schedule records and 160 properties per serial run, including configured/runtime target distinctions, copying, binding, unsupported schedules and rollback. Both fresh focused executions match exactly; all 367 Application cases and two-rank schedule drift plus every two/four-rank transaction consensus participant pass with no skips. The combined source includes WP-4 producer observations through `6c209893`. Source reviews and independent verification of eleven XML files, all 4,670 inputs and retained binaries passed; the prior incorrect new diagnostic expectation and failed run remain recorded. The baseline manifest links acceptance. Driver consumption, immutable configuration/runtime separation and six lifecycle candidate comparisons remain pending.

**Progress, 2026-09-05 (UTC), driver migration:** The driver now consumes the accepted immutable maintenance configuration at both original setup points, with mutable workflow state and declaration bindings stored separately. Duplicate maintenance parsing is removed; this slice reduces the driver by 899 lines. Source review, the ownership case, twelve focused cases and their repeat, all 368 Application cases and selected two/four-rank checks pass with no skips. All 52 complete schedules match the accepted before-data exactly. The actual candidate build retains 4,670 source inputs and unchanged options for all 1,269 common compile recipes. Fourteen lifecycle capture groups produced nineteen successful executions, sixty-six expected refusals and no skips; independent verification confirms exact baseline payload parity and byte-identical repeats for all six fixtures under the frozen policy. Only the separately checked source provenance values differ. The baseline manifest links the acceptance, reviews, retained source/binaries and failed generations. Cut-option consolidation, canonical input-path reconciliation, physical adapters and complete effective-policy provenance remain open; R1 and broader physical qualification are not complete.

**Progress, 2026-09-05 (UTC), cut-option conversion:** The driver now delegates active-cut and maintenance option projection to two pure functions in `Application/Core/LevelSetCutConfiguration.h/.cpp`, reducing its assignment code by 84 lines. Complete old/new options and effective-order/failure records match exactly, including omitted versus explicit quadrature orders, distinct maintenance identity and deferred FE validation. After that comparison, both temporary Driver observations and their support header were removed. Four typed configuration tests and two real Driver refresh tests replace the temporary property cases. Source reviews and independent verification pass: six focused cases, all 374 Application cases, 345 FE level-set passes with the one pre-existing release-matrix skip, and every selected two/four-rank participant. All 4,670 inputs, eleven XML files and four retained binaries are recorded in the acceptance linked by the baseline manifest. Canonical input reconciliation, physical adapters, public option/default consolidation and complete policy provenance remain open; this is a bounded R1 extraction, with no new physical qualification claim.

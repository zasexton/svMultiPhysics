...
**Progress, 2026-09-04:** The Physics option extraction and resolved level-set translator slices passed source review and configured verification; they are committed in `b60221f9` and `d4907a49`. The translator has 23 focused passes and 309 integrated Application passes/four Eigen skips, with 51 focused Physics passes/one JIT skip. Builder reuse passed its nine focused cases after test-fixture corrections. Current-branch integration through `45864916` passed 316 Application cases, two/four-rank consensus on every rank, 274 FE Constraints cases and 269 FE TimeStepping cases with one existing skip. Exact source, binaries, selections and evidence are recorded in the baseline manifest. Maintenance/install behavior is now frozen by 17 characterization passes; the current pre-extraction Application check passed all 335 cases and two/four-rank consensus with no skips, on `4b2ee16b`. Retained input snapshots, driver compatibility views, cut-option consolidation, physical input adapters and effective-value provenance remain open; R1 is not complete.

...
- **Current work:** R0 baseline execution and test-only operator capture; R1 resolved configuration. The Physics free-surface option declarations now live in `Physics/Formulations/NavierStokes/FreeSurface/FreeSurfaceOptions.h`, with aliases preserving existing nested C++ names and direct reuse of `FE::constraints::SmallCutAggregationGuardOptions`. Static review, five syntax checks and configured baseline/candidate checks passed: 135 cases passed and one JIT-only case skipped in each run. Defaults and assembly formulas are unchanged. The next slice exposes the existing level-set translation through `Application/Core/ResolvedMovingDomainConfiguration.h`; its installation, dependency and preregistration overloads preserve current layering. Driver maintenance and provenance migration remain pending.
...
- **Compute ownership:** Only Slurm job IDs entered at submission in [the refactoring job ledger](free_surface_refactor_job_ledger_20260904.md) may be modified by this work. The allocation ceiling is four nodes and 80 GB aggregate requested memory on `amarsden`, including pending reservations. Current reservations are `42080271` (JIT baseline), `42082703` (unchanged core binaries with corrected fixture directory and MPI slots), and `42083671` (frozen resolver candidate), totaling three nodes and 54,000 MiB. Option candidate `42080275` completed successfully. Core `42068077` retained setup failures; its Physics selection has 466 passed, 25 skipped and zero failures. Failed JIT attempts remain recorded. Current state and evidence are in the ledger.

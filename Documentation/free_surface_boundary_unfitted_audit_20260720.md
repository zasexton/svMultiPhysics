...
- **Current work:** R0 has accepted bounded serial/two-rank wet-block and six one-rank lifecycle references, with broader capture pending. R1 driver state migration preserves all 52 schedules and six exact lifecycle payloads. The new cut-option slice delegates active and maintenance projection to `Application/Core/LevelSetCutConfiguration.h/.cpp`, preserves all actual old option/order/failure records, and removes its temporary observations. Final checks pass all 374 Application cases, 345 FE level-set cases with the one original release-matrix skip, and all selected two/four-rank participants. Canonical input reconciliation, physical adapters, public option/default consolidation and complete effective-policy provenance remain open.
...
- **Compute ownership:** Only Slurm job IDs entered at submission in [the refactoring job ledger](free_surface_refactor_job_ledger_20260904.md) may be modified by this work. The ceiling remains four nodes and 80 GB aggregate requested memory on `amarsden`, including pending reservations. The remaining reservation is successor verification allocation `42161975`: one node / 32,000 MiB. Allocations `42093152` and `42130677` completed normally after all of their numeric steps became terminal. Enabled-feature baseline `42090895` completed its 20 declared groups without failures; exclusions and individual skips remain recorded. JIT profile `42080271`, resolver candidate `42083671` and corrected serial/MPI baseline `42086668` completed successfully. Earlier failed configure/build/launch attempts remain recorded; skips and incomplete operator/history captures are not treated as qualification passes. Current state and evidence are in the ledger.

- **Next shared-file extraction:** Prepare an FE-only scalar reaction-diffusion reference in the existing `FE/Tests/Unit/Systems/test_FormsInstaller.cpp` owner, then expose a thin `FE/Forms/IntegrationDomain.h` volume selector over existing measures. Preserve selected versus retained support, marker/side identity, exact dry zeros, full-cell reuse, currentness, DOF order and standard/JIT results. The original-reference and temporary interpreter-rejection builds are private evidence overlays; no permanent JIT guard is planned. Physics form/domain migration remains separately gated.
...
| 2026-09-05 (UTC) | R1, maintenance producer and shared parsing | Accepted typed compatibility configuration and source-backed chronological velocity provenance. Seventeen producer, seven snapshot, twenty-one compatibility and 364 full Application cases passed, with every two/four-rank consensus participant and strict JSON repeats; no skips. Reviewed evidence and failed generations are linked in the baseline manifest | Driver consumption/runtime bindings, cut options, physical input adapters and complete effective-policy inventory remain open |

| 2026-09-05 (UTC) | R1, driver characterization | Three test files preserve 52 complete schedules, binding/target/copy/rollback state and public setup-error order. Twelve focused cases repeat exactly; all 367 Application cases and selected two/four-rank checks pass, no skips, on retained `ea8fb633` including merged WP-4 observations | Production driver migration and six lifecycle candidate comparisons remain pending; no physical qualification claim changes |

| 2026-09-05 (UTC) | R1, driver state migration | Accepted immutable configuration consumption and separate runtime/bindings, removing 899 driver lines. Source review, all 368 Application cases and selected two/four-rank checks pass without skips; 52 before-data schedules and all six frozen lifecycle payloads match exactly, with fresh repeats and retained source/build provenance | Cut options, canonical compatibility decisions, physical adapters and complete policy inventory remain open; no broader physical qualification claim |

| 2026-09-05 (UTC) | R1, cut-option conversion | Accepted two pure Application-to-FE adapters and Driver delegation, preserving complete old/new option records and failure timing. Temporary observations removed; six durable tests, all 374 Application cases, 345 FE level-set passes/one prior release-matrix skip and selected MPI participants verified | Canonical input behavior, physical adapters, option/default consolidation and policy inventory remain open; FE-only scalar domain characterization is next |
...
WP-4 private producer arithmetic checkpoint on 2026-09-05 (reviewed development prerequisite, not qualification):

- Commit `cfe8ca720cfa01a165848b0c14c5a3725f6a68f7` adds a private compiled fixed-capacity integer backend for binary64 interval bounds, original-input edge-root assessments and origin-aware distance margins. It changes no producer geometry, derivative consumer or public construction-success state; the two producer capture adapters are the next dependent task. Unsupported representation, range and unresolved branch/margin cases remain unavailable.
- Retained primitive and composed-operation runtime RED phases cover the intended unavailable behavior. An exact-subnormal range regression exposed and corrected an equality-path omission. Final focused execution controls pass 19/19; the complete geometry target passes 302/302 tests in 30 suites, with no failures, errors, skips or disabled cases. A separate scratch allocation-count probe passes eight repetitions. Integer-only object/call-graph inspection, bounded nonrecursive stack analysis and eager symbol-binding inspection support this GCC12.4/x86-64 static-library executable configuration; another linked consumer remains unqualified until its actual binding is inspected.
- Evidence `/scratch/users/zsexton/wp4-producer-arithmetic-kernel-20260905-jurIRA` binds source `b71d132c` plus reviewed patch SHA-256 `5112cfcd51768e35020deab3dac5c6f69ef7546f4d3de204872854abe29ed752`, whose four-file contents are committed in `cfe8ca72`. Final executable SHA-256 is `6d93d0fe1d6d8a8094fbf4ef0e87879fabb516bcead41e8fded8ba33386e33a7`. Independent checks cover all 3115 non-protected participating Code files, recipes, object/executable and per-phase raw-index preservation. Original direct-extern routing, later explicit bounded steps, failed phases and inherited warning limits remain recorded, not relabeled. The protected uncompiled diagnostic is still dirty, so this is incremental development evidence, not a clean qualification freeze.
- The exact patch/evidence review accepts specification and quality with no Critical or Important findings. Its Minor report-only stack estimate was corrected without a numerical rerun. Ordinary merge `b501460543d4456cc2cf3559d141677c348c72fd` preserves the concurrent cut-option conversion refactor and coordination roadmap; the FE tree is unchanged by that integration, but no new combined Application-binary claim follows. Existing allocation `42053782` supplied the checks; no new job or physical replay was needed.
- This completes only the arithmetic-kernel prerequisite. Adapter integration, quantitative geometry/scalar certification, current-coefficient derivative binding, kinematic compatibility and corrected physical convergence remain open. Task 4, WP-4, FSR-03, FSR-04 and Q2 remain unchecked; qualification gates, thresholds, rejected evidence and all checklist boxes are unchanged.

WP-4 producer arithmetic integration checkpoint on 2026-09-05 (reviewed development prerequisite, not qualification):

- Commit `721599682359d52b9925f432cd5a1a20827b0741` links original inputs and actually consumed edge/root operations to the private arithmetic backend in the existing cell and active-boundary producers. It carries call-local point assessments through existing geometry operations and evaluates each legacy distance predicate once. A kernel-local correction reuses only bit-identical scalar/norm operands; no geometry, force, coefficient policy, tolerance or public success state is changed.
- Real-producer runtime RED, corrected focused 30/30 and a scratch-only private-call 2/2 check establish original-input linkage and available non-dyadic assessments in the actual cell/boundary paths. The private check retains 153 edge and 1884 distance records, no capture overflow, and explicit caller/fixture attribution. Its executable uses the pre-optimization kernel and remains historical evidence; unchanged producer objects, exact-duplicate semantics, changed-object binding inspection and the existing 19 kernel/state checks support the stated carry-forward scope, not a new private run.
- The initial integrated full geometry run timed out at 300 seconds after 143 completed tests; its sphere refinement took 206.287 seconds. After the bounded duplicate-elision correction, focused tests pass 30/30 in 0.63 seconds and the complete normal geometry suite passes 304/304 in 30 suites in 276.988 seconds, with no failures, errors, skips or disabled cases. Sphere and curved-normal refinement take 136.961 and 137.450 seconds. The unchanged 300-second limit has only 23.012 seconds of margin; baseline performance is not restored (the earlier kernel-only sphere took 0.067 seconds). This remains a material runtime limitation, not evidence supporting an expanded three-dimensional campaign.
- Evidence is retained at `/scratch/users/zsexton/wp4-producer-arithmetic-adapters-20260905-ZjRxEo` and `/scratch/users/zsexton/wp4-arithmetic-duplicate-elision-20260905-OxZnoX`. Latest five-file patch SHA-256 is `60b8f7d206e1915cd2c43d057fa45790f382175a381f44cb6eefe2cea39437ee`; normal executable SHA-256 is `56c73241a838c7d3c969d7d3a0748ad0c19b55019c9f5192bec80f6c34df2081`. Independent checks cover all 3115 participating non-protected Code files, recipes, products, actual raw index/entries/HEAD/status guards and all 16 original structural geometry records byte-for-byte. Existing allocation `42053782` supplied steps 58 and 59; no new job, timeout change or physical replay was needed.
- Tetrahedral symbolic face reuse and represented-value canonicalization remain independent `ModifiedOrUnresolved` observations. Triangle/triangle-wall positive controls remain `Unchecked`; no strict three-dimensional construction success is claimed. The 16 retained records do not cover full negated tetrahedral-cell output parity. The protected uncompiled LevelSet diagnostic is unchanged and unstaged, so this is incremental development evidence rather than a clean qualification freeze. No new integrated Application or Physics binary claim follows.
- An independent read-only captured-case premise check also ruled out a coefficient-policy-only repair: under hypothetical coordinated exact-zero classification, actual cell 919 has selected negative fraction about `4.8e-22`, below the unchanged `1e-8` retention threshold, while its interface survives. The represented retained-volume derivative is locally zero but the ordinary unpruned kinematic mass is nonzero. This conditional derivation is not an executed new-policy test or full causal attribution; it identifies the additional retained-domain boundary/incidence and deformation contract required for the next complete production repair. The earlier accepted-stage prescribed-angle premise remains conditional and does not establish endpoint-anchor preservation or maintenance work.
- The exact five-file review accepts this bounded prerequisite with no blocking findings; report SHA-256 is `e906d7940c644121f74ecb9a99b43f4ee811ca9a98041cacadf5539455de0b20`. This closes only the stated adapter/verification prerequisite. Current-coefficient binding, quantitative geometry/scalar consistency, retained-domain kinematics, corrected production convergence, prescribed-angle completion and all qualification obligations remain open. Task 4, WP-4, Q2, FSR-03 and FSR-04 remain unchecked; no checklist box, physical gate or rejected qualification evidence changes.

WP-4 producer arithmetic performance follow-up on 2026-09-05 (reviewed development prerequisite, not qualification):

- A three-line exact-zero multiplication-row elision preserves the existing validation, carry and result behavior. It removes measured redundant integer work without changing producer geometry, observations, force, coefficient policy or any tolerance. Focused tests pass 30/30 and the complete geometry suite passes 304/304 in 30 suites in 170.359 seconds, with no failures, errors, skips or disabled cases. Sphere and curved-normal refinement take 84.394 and 84.369 seconds. Relative to the preceding single 276.988-second run, full-suite time decreases by 38.50%, leaving 129.641 seconds under the unchanged 300-second limit; this is not a timing distribution or restoration of pre-adapter throughput.
- Evidence `/scratch/users/zsexton/wp4-arithmetic-zero-rows-20260905-nLaPfZ` binds base `9489a50157bec48f11bf81795e6884ca3af585f3` plus one-file patch SHA-256 `01875dd24b31f38b7b7a19054fb0808fb80ac32d96ed2612231aa4e58307411a`; executable SHA-256 is `8266d68379d8c601b597a66d99e4284b1e61ec674ac6b244be656d3ccc32f0d8`. Existing allocation `42053782` supplied steps 60 and 61; both guards returned zero and released. Independent checks cover 3115 participating non-protected Code files, all 13 recipe-manifest entries, current products, metadata guards and all 16 original geometry records in both phases. Historical packet wording said 14 recipes; the explicit manifests and wrapper lists contain 13, all verified, with no missing listed input or repeated execution.
- Scoped review SHA-256 `af02a656a7f38fbac321b26d1a7d71fc321af70f2c40bfb3dae12619471753e4` accepts the exact correction with only that report-count clarification. This removes the immediate geometry-suite timeout bottleneck for subsequent focused repair work. It establishes no new integrated Application/Physics result, strict three-dimensional success, captured-case convergence or qualification; the retained-domain/scalar/derivative/kinematic repair and every existing open gate remain required. The protected diagnostic and all checklist boxes are unchanged.

WP-4 direct dyadic-bound conversion checkpoint on 2026-09-05 (reviewed development prerequisite, not qualification):

- The private integer backend now rounds exact dyadic sums/products directly from their leading significand and discarded bits instead of searching binary64 candidates. The accepted bounded proof and independent integer oracle establish the same interval endpoints and range failures, including nonzero-subnormal rejection. Division, square root, multiplication storage, producer observations, geometry, force and tolerances are unchanged.
- Corrected focused checks pass 34/34; the complete geometry suite passes 308/308 in 30 suites, with no failures, errors, skips or disabled cases. Like-for-like Google Test time falls from 170.359 to 37.037 seconds (78.259%); command time falls from 170.37 to 37.05 seconds. Sphere and curved-normal refinement take 18.055 and 18.543 seconds. All 16 retained structural geometry records remain byte-identical. This is a measured development improvement, not a timing distribution or restoration of baseline throughput.
- Evidence `/scratch/users/zsexton/wp4-dyadic-brackets-20260905-iRv9Da` binds base `573fb4d2a167407940f21f4b65f1c4f28f4e05a9` plus exact two-file patch SHA-256 `323a904347dfe1732b8aa4590fdb4e0f31b14bd1a7a16385cab6983126b8488b`. Final executable SHA-256 is `d0f8444a03ba61a0ddb93b0a014f1c6bdae0855fd77e27d3f34d7346afb67a23`; changed-object inspection retains integer-only operations and no new runtime dependency. Corrected steps `42053782.69` and `.70` passed and released every source/recipe/product/raw-index/protected-file guard. A preceding scheduler-query timeout occurred before any build or test; its record is preserved. No new job or timeout increase was needed.
- Independent scoped review caught an initial oracle operand that exercised exponent overflow instead of the required maximum-finite tail. The test-only correction adds exact-integer setup assertions and passed the corrected focused/full round; the production helper was unchanged. The final implementation report is bound by SHA-256 `a2319c0f0a13d166db5071b1938954f35bfc65f70d9d51888948fdfd37738663`; accepted review SHA-256 is `1571262aa736b9b24d284e45842c057d9c52b43592d8801348c9389ffc785506`. Review accepts the corrected FE prerequisite; its timing-label clarification is reflected in the consistent comparisons above.
- The separate current-candidate producer-linkage patch has focused serial/two-rank evidence, but its initial complete Application invocation timed out at 600 seconds after 72 completions and 73 starts from 375 registered tests, without XML. That failure remains preserved at `/scratch/users/zsexton/wp4-current-candidate-binding-20260905-ZbTWIz`. Combined Application verification on this corrected arithmetic dependency is the next consumer; this FE result does not supply that missing pass or complete authoritative derivatives.
- Conditional retained-boundary analysis also shows that local interface elision alone omits two retained/pruned parent-face fluxes on the captured 512 star. Subsequent inspection found that the existing records lack field-labelled closed physical velocity/pressure constraints and exhaustive reverse dependencies, so neither admitted-witness membership nor constrained-space exclusion is established. No new-policy run, force mask, physical repair or convergence claim follows. The protected uncompiled diagnostic, rejected evidence, every checklist box, and all open WP-4/Task 4/FSR-03/FSR-04/Q2 qualification obligations remain unchanged.

WP-4 combined Application and prescribed-strip checkpoint on 2026-09-05 (development evidence, not qualification):

- Central commits `d319c390` and `93aa87ae` integrate the reviewed dyadic bounds and preserve the concurrent architecture history through `55cff27c`. Combined evidence at `/scratch/users/zsexton/wp4-dyadic-application-20260905-fGMAjk` binds HEAD `93aa87aef9273d5a4b8ce30af9675f17ed4a98f7` plus the unchanged, uncommitted three-file current-candidate linkage patch `c5b9ac04c36ab6753ff417bc4a4e4eff8e9b76efdb6bfeab24149fd3983cd6b8`. The existing solver, serial test and MPI test rebuilt successfully in 31.05 seconds; all three bind the accepted arithmetic object `72236c2bcf1867dcbb9bd843648fd95640ea85c5e2a06a2781289549e39723fe`. This is new consumer-link evidence, not a repeated arithmetic proof.
- The required complete single-rank Application run still failed its unchanged 600-second limit: 375 registered, 76 started, 75 completed passes, 299 unstarted and 300 unfinished, with no XML. It stopped in the hydrostatic-gravity/fixed-pressure-gauge control. Completed minimized circle/sphere/cap time fell from 321.332 to 74.940 seconds; sampled GCI took 242.412 seconds, and the five-angle matrix 37.730 seconds. These are individual measurements, not a full-suite pass or timing distribution. After intact intermediate guards, the exact two-rank MPI regression passed 1/1 in 3.10 seconds, with zero nonpass cases.
- Existing allocation `42053782` supplied configuration step 71 and combined step 72; no job or timeout increase was needed. Configuration returned zero in 5.90 seconds with unchanged cache and all 61 consumed recipes. All seven prior products were preserved; every combined source/input/recipe/raw-index/product/protected-file guard returned zero and released at 23:52:51 UTC. The failed wrapper result remains one. Final execution report SHA-256 `d29060bf02d86d01808e16866f841427a32d379489790b09e5809fcc063bb27a` and scoped review `c1e6c9ed7ef9da148ce7a3e35fc6261f7a35c27c01ae91a2cae29a3fa1871c7e` retain insufficient full evidence, with no new code or binding finding. The linkage patch remains uncommitted. The measured blocker is serial numerical cost; a bounded check of compact division/square-root replacements is next, not repetition of this unchanged invocation.
- Independent prescribed-strip derivation SHA-256 `c996745f3db853453a056220d9baa19fa9415505ba6aaf31cf2cef9559381f43` resolves one separate representation premise: on its explicit connected two-tetrahedron/five-DOF continuous P1 fixture, fixed bent contact anchors and two ninety-degree angle targets have sharp minimum worst-cell angle error `atan(1/2)`. Relaxing unit-gradient target assignments alone cannot repair that fixed-space incompatibility. A mathematical extra-affine-break example does not authorize production mesh/anchor changes or establish a general curved-strip method. This manufactured theorem is not the captured production case.
- No full Application acceptance, corrected production pilot, physical-method change or qualification follows. The protected diagnostic, prior rejected evidence and every audit checkbox remain unchanged; WP-4, Task 4, FSR-03, FSR-04 and Q2 remain open.

WP-4 compact quotient/root checkpoint on 2026-09-05 local date (reviewed development prerequisite, not qualification):

- The private division and square-root paths now use bounded normalized-significand integer recurrences instead of wide candidate searches. Accepted exact arithmetic reasoning and four independent integer-oracle tests preserve adjacent endpoints, range failures, validation order, signs and zeros. Public nonzero-subnormal rejection is unchanged; private subnormal preservation has source/theorem support, not a new runtime seam. Producer geometry, observations, coefficient policy, force and all tolerances are unchanged.
- Evidence `/scratch/users/zsexton/wp4-compact-brackets-20260905-qngaKA` binds base `62b529956d5255b4029354a4e634d024723be410` and two-file patch SHA-256 `356dd4f8f415539ba584e857ebd0c5180bf7956658419d305e84c584d6022a67`. Focused checks pass 38/38; full Geometry passes 312/312 in 30 suites, all run/completed with zero nonpass cases. All 16 retained structural records remain byte-identical. Full command time falls from 37.05 to 6.83 seconds; Google Test time falls from 37.037 to 6.809 seconds. These single-run observations do not establish restored baseline throughput or an Application speed guarantee.
- Existing allocation `42053782` supplied steps 73-75. Same-options configuration took 3.57 seconds with unchanged cache and ten consumed recipes; the single focused build took 23.56 seconds. Full testing reused the focused products without rebuilding. All source/cache/recipe/raw-index/product/archive/protected-file guards returned zero and released by 2026-09-06 00:27:43 UTC. The previous products and failed Application evidence remain preserved. No new job, test omission or limit increase was needed.
- Final executable SHA-256 is `781a4391b3537a43befb7a2971d37056c8bd5c835bd8e4d7f78c2874853f2a13`; arithmetic object SHA-256 is `d4aef1d80bf10138dedcc830ee39bd70edb752cd77526ce3b9cdf58d8b4920a0`. Changed-object inspection finds no added runtime dependency or floating operation. Changed-path stack observations are not whole-program bounds. The retained unused generic-search helper emits a disclosed non-blocking warning. Implementation report SHA-256 `8ed13aafa327615dbaa04b854a21f8c66d020dd418e0198ad391ee4c96a03818` and independent review `575fcc392e7c3420f393c8df269bd54214ef4192c15204cfd39475b395090bf6` accept this exact FE prerequisite; the review distinguishes the unchanged unmasked-multiply test from executed quotient/root state coverage.
- This removes the remaining scalar-search cost targeted by the prerequisite. Its next consumer is the still-missing complete 375-test Application result and exact two-rank linkage regression on newly linked products under the unchanged 600-second gates. Current-candidate linkage remains uncommitted until that evidence passes. The captured geometry/scalar/derivative/kinematic repair, corrected production convergence, prescribed-angle work and qualification remain open. No audit checkbox or protected diagnostic changes.

WP-4 current-candidate linkage acceptance on 2026-09-05 local date (reviewed development prerequisite, not qualification):

- The existing Application geometry transaction now checks the actual constrained candidate against its current refresh signature, active context, request/layout/cache identity and reported producer source before exact functional projection. The checks use the existing owners and collective sequence, without a new persistent coefficient copy or digest service. Stale content or source linkage is rejected; this precondition alone does not establish authoritative FE derivatives or repair canonicalized/pruned geometry.
- Preserved focused RED/GREEN evidence and source review `412f786681fadb8d80e5ceecfa431d6984260901d5e24cc6b49728426c2ef8d6` cover the unchanged three-file patch `c5b9ac04c36ab6753ff417bc4a4e4eff8e9b76efdb6bfeab24149fd3983cd6b8`. New combined evidence at `/scratch/users/zsexton/wp4-compact-application-20260905-wfyhzR` binds HEAD `f454dfd78dc91685d4942b4f77b208cf6dd003cd` plus that patch. The solver and serial/MPI tests rebuilt and linked the accepted compact arithmetic object successfully in 25.61 seconds. No unchanged configuration, focused serial or FE run was repeated.
- Full Application now passes 375/375 in 30 suites, including the current-candidate truth table: all tests run/completed and zero failures, errors, skips or disabled cases. Command time is 364.36 seconds under the unchanged 600-second cap; Google Test time is 362.011 seconds. The separate exact two-rank linkage test passes 1/1 in 2.96 seconds, with both ranks accounted for. Hydrostatic initialization is the measured dominant serial case at 267.486 seconds; sampled GCI takes 47.437 seconds and minimized circle/sphere/cap controls 13.192 seconds. This removes the previous full-verification blocker, not all future runtime risk.
- Existing step `42053782.76` ran from 2026-09-06 00:46:42 to guard release at 00:54:30 UTC. Complete source/input/cache/recipe/raw-index/new-product and preserved-old-product guards all return zero. The new seven-product manifest is `5a438c5d4eab0b401056412e3d1ec0e725ef3d035331a053704dbd9d8fb3d773`; all three consumers define the required arithmetic assessments. Final execution report SHA-256 `62aaa2917a2b87595d17d9fe975fec414e019c7a165cc373766ad246499820d2` and scoped review `5a1cbb84063dbe4e28b32fadd1312fc2ea43f6cda3b721f525f541cbf909dc9a` accept this combined evidence with no blocking finding. Previous failed attempts and binaries remain preserved.
- Current-candidate linkage and its required verification are complete in this bounded scope. The next production obligation is the smallest joint geometry/scalar/derivative/kinematic repair for the captured failing case, including the unresolved retained-domain contract; rejecting that state is not its repair. No new production minimizer run, changed physical gate, qualification launch, protected diagnostic edit or audit checkbox change follows. WP-4, Task 4, FSR-03, FSR-04 and Q2 remain open.

WP-4 physical-constraint capture on 2026-09-05 local date (accepted diagnostic records, not qualification):

- Evidence `/scratch/users/zsexton/wp4-physical-constraints-20260905-lNzwpv` binds committed source `eb144d4f80d44474817c171eb7c9d738ea8c62f2` and private probe SHA-256 `97730c5e3d9833a9893e12612dea8beb51c90f808af64b0aa4398dfb280a96c6`. One production-transaction capture reproduces exactly the earlier base and negative full-step sample's arrays, maps and scalars, followed by semantic rollback. The negative sample remains a legacy-policy diagnostic state, not an accepted solution or an exact-zero-policy execution.
- The 23-file payload manifest is `48a6d34e6358a940f42beb9cd361ec2770d3b2c85584c5d3fe581ee20f88df4e`. Each state includes all 6,724 field rows, 4,452 closed affine slave lines and 420 terminal-master entries, original mesh connectivity, stored reference subcells, binding revisions and retention data. Physical vertex 512's y-velocity is free row 2432 but has two reverse-dependent slaves, 2430 and 3008; a free central row alone therefore does not establish compact-hat admission. The negative state's ledger has 12 pruned rules; the base has none.
- Existing allocation step `42053782.81` passed the single two-state test, with zero failures/errors/skips/disabled cases, in 5.17 seconds command time and 287,372 KiB peak RSS. All source/cache/recipe/input/raw-index/private-product/dependency guards released intact at 2026-09-06 02:26:57 UTC. The corrected private build took 198.35 seconds and 2,992,540 KiB peak RSS. Earlier private compilation and input-reader failures remain preserved and are not physical-regression failures; no full suite, production minimizer or qualification was repeated.
- Readback found four malformed redundant reverse-dependency annotations, two per state. Their complete field/constraint tables are unaffected and supply the exact exhaustive reconstruction; the original payload remains unchanged. The downstream reader also encountered a simplex vertex count emitted as raw byte 3 rather than decimal text, stopping before a scientific result. That lossless encoding needs an explicit pinned reader accommodation, not a new geometry capture. Both limitations remain recorded rather than treating the initial payload-schema check as exhaustive.
- Capture report SHA-256 `b72f458a12162d98f8e424e93757cae1a35b0b1d94ccfd8bb69f9c8053d7547f` and scoped review `9507acd3d0305455a8f814b942821c21d9124341cc1125086e18611a10795de0` accept the physical record with the annotation limitation. This removes the missing actual physical-row/support data. The next consumer is the bounded admitted-space/retained-face calculation; no nonzero physical witness, full assembled pressure identity, repaired production convergence or qualification follows yet. All audit checkboxes and protected diagnostics remain unchanged.

WP-4 admitted-space and complete-boundary check on 2026-09-05 local date (bounded scientific evidence, not qualification):

- The original captured vertex-512 compact velocity hat and literal active-pressure constant do not satisfy all stored closed affine equations. A different explicit pair does: free velocity row 2501 is one, its two induced rows retain the captured exact weights, and all 197 free pressure coefficients are one with the same affine prolongation. Both complete residual maps vanish in exact rational interpretation of the stored binary64 equations. This is not a rounded-distribution or universal-space claim.
- Accepted calculation `/scratch/users/zsexton/wp4-actual-retained-space-20260905-2Is2Mt/analysis.json`, SHA-256 `038981338e167b8c9c14721c27e189638fe3525eb8ee1827fd03b6c49ce7240f`, establishes a nonzero retained/pruned parent-face subterm for that pair. The pinned target-annotation and raw-byte count readers preserve every captured byte; failed reader attempt 82 remains separately archived. Step 83 completed in 2.11 seconds with intact released guards.
- The fixed-pair extension `/scratch/users/zsexton/wp4-full-parent-boundary-20260905-2KOXi3/analysis.json`, SHA-256 `585e3c43d99bfc1b5c6d43d7cfa9dc4231e01116a579264c457a63bd79afd627`, accounts for all 24 internal edges and 48 incident sides on the 13-cell velocity support. Source-backed whole-parent interpretation resolves the missing full-cell traces without recutting. Three edge totals are nonzero and 21 are zero; there are no unresolved trace limits or exterior-support edges. The complete exact internal sum is `-1524509155428733645822140679628711597529484947283/3369993333393829974333376885877453834204643052817571560137951281152`; retained/pruned and retained/retained contributions are recorded separately.
- Step `42053782.84` completed in 1.08 seconds, with 44,224 KiB peak RSS and command/wrapper/final-guard exits zero. All guards released. Report `5b72e0ba56a81900e12064201bdf10fc340687d8535399fa928f2470019c71da` and scoped review `0789233a1e09b94db0e9c89dcf7729f271b18404150381485f598003930cfb2a` accept only this fixed-witness represented-domain result. It is not an error enclosure for rounded quadrature, actual assembled pressure work, coefficient/material equivalence, or a quantitative explanation of the minimizer failure. Preparation and review, not the roughly one-second calculation, are the measured current bottleneck.
- This resolves the specific full-boundary cancellation question. The next implementation dependency is the already designed explicit coefficient policy shared by cell and wall producers, while the joint retained-domain/derivative/kinematic repair remains required before a production pilot. No physical gate, protected diagnostic, audit checkbox or qualification status changes. The supplementary Gross and Reusken (2011) reference is indexed in the scratch continuation notes; its licensed PDF stays outside Git.

WP-4 explicit coefficient-policy acceptance on 2026-09-05 local date (reviewed development prerequisite, not qualification):

- Cell and wall producers now share a request-owned choice between the unchanged legacy absolute band and opt-in exact represented zero for linear-corner geometry. Linked active/contact rules, snapshot identity and distributed revision agreement bind that choice and its resolved band. Physical distance, measure, retention, quadrature, force and normal gates are unchanged. This removes the inability to express and identify the intended coefficient-classification policy; lifecycle adoption and the joint geometry/scalar/derivative/kinematic repair remain separate work.
- Evidence `/scratch/users/zsexton/wp4-coefficient-policy-20260905-hqymCX` binds base `7054ae63b56a89c29844154206d48603fd17bfbf` and twelve-file patch SHA-256 `4882b4fa7db73145994dc760ba19b1cfff46733a769c0d0da4b63d280106a291`. The intended RED completed eight tests with four behavior failures and four controls. Corrected GREEN passes 8/8, including real triangle, tetrahedron and wall cuts at scales `1e-14`, `1` and `1e14`; six selected legacy records remain byte-identical. Full Geometry reuses the same products without rebuilding and passes 320/320 across 31 suites, with zero failures, errors, skips or disabled cases. Full XML SHA-256 is `c349576fd2e3ab108aed4347698508b21fb4f3c9a6aab9883c997aba7b62da8d`.
- Existing steps `42053782.85` through `.88` supplied the evidence; no new job was submitted. The first GREEN attempt was a compile-only overload failure with six partial cache outputs, not a test pass or clean guard release. Its products and failed outcome remain preserved under the explicit coordinator disposition `8126fbbbf6dd6c50d069c501b24920d4d6e80639310d31788f88d7b0e95bf337`; the sole source correction types the existing hash tag as `std::uint64_t`. Corrected GREEN and full-suite guards all pass and release by 2026-09-06 04:41:12 UTC. The changed-header rebuild took 366.14 seconds, versus 0.03 seconds focused testing and 8.08 seconds full testing. Compilation and the separately recorded preparation/recovery coordination, not queue delay, dominate this prerequisite.
- Implementation report `.superpowers/sdd/plan_wp4_balanced_force_completion_20260903/task-4-coefficient-policy-report.md`, SHA-256 `bdc97028b63ced19c69b2b10796de47079b0613f00bcb2368dd8e9596021f922`, and scoped review `task-4-coefficient-policy-review-report.md`, SHA-256 `a94c5631bef4ec63df56b0ed9fd32b88fe6a28a756eb77b24034f263fc43c5ca`, accept this exact low-level seam. The changed public header means earlier Application/Physics binaries do not verify this source. Prescribed-angle working-scale regressions are prepared independently but not executed. No production repair, qualification, protected-diagnostic change or audit-checkbox closure follows; WP-4, Task 4, FSR-03, FSR-04 and Q2 remain open.

WP-4 prescribed working-state integration on 2026-09-05 local date (reviewed development repair, not qualification):

- Changed-dependency Application evidence at `/scratch/users/zsexton/wp4-coefficient-policy-application-20260905-Gk6WJO` now verifies B3 at `fe26d3655a2baa41265fdc34cf92ee612d4d4b9f`: 375/375 serial tests in 30 suites and the selected test on both MPI ranks pass with zero nonpass cases. Build time is 878.07 seconds, serial 377.62 seconds and MPI 2.94 seconds. All source, recipe, index, 144 rebuilt-product and preserved-product guards released at 2026-09-06 05:38:16 UTC. Report SHA-256 is `00e6479e857132b711dc5643731e9d70c49d9a74069344a02b30ea31aa5d5a28`. This is evidence for the preceding B3 source, not the subsequently integrated reinitialization change or a separate Physics result.
- Prescribed-angle repair now selects its working path from globally gathered constraints, discovers original cut support using the explicit exact-zero policy and local coefficient ratios, assembles the existing target, and applies one safe positive common fit. Canonical coefficients supply relaxation, motion and normal checks; owner-authoritative protected values are restored only at publication. Pure bulk and dynamic-only numerical branches, physical tolerances, target priority and success-plus-convergence gates are retained. This removes the demonstrated tiny-amplitude support loss and raw-amplitude dependence of finite relaxation; it does not select a new target or certify production lifecycle geometry.
- The unchanged eight regressions first completed with eight failures and 519 intended assertions. At scale `1e14`, finite relaxation left residual `6.4565616513245772e-6` against the unchanged `1e-12` gate; scale `1e-14` lost cut support. The repaired source passes all eight, then all 27 unprotected same-file tests, with zero nonpass cases. Private evidence `/scratch/users/zsexton/wp4-prescribed-working-scale-20260905-1JxTj9` binds source patch `73e92844b11d74c76e7be6f7f13c7e487ed2659fbed9a5d0cf99d44f6753b072` and unchanged test patch `b04fb10d58c1959b3298aa60dfd964494aabdcd99b11121cffadfd19d2d933b1`. The private archive replaces exactly one of 424 members, preserving every other member and the existing FE cache. Build time is 27.90 seconds; focused testing is 0.04 seconds and reused-product same-file testing is 0.08 seconds. Steps 91 and 92 released all guards by 05:49:36 UTC.
- Distributed evidence `/scratch/users/zsexton/wp4-prescribed-working-scale-mpi-20260905-joWVQV` preserves step 93 as a pre-test launcher failure: compilation succeeded, but neither rank launched because the one-task step exposed one MPI slot. Step 94 reused those exact binaries with the established local MPI slot setting and passed both existing tests on both ranks in 1.95 seconds, including zero inter-rank coefficient difference. Rank-separated XML and all guards released at 06:06:28 UTC; the non-fatal network-filesystem shared-memory warning remains disclosed. The complete successful-phase manifest is `032de024e16e1e5060749c9bfcc489d5d53c01d8aa9fa3c0e4d4daee1b3ef843`. No new allocation, source adjustment, test change or recompilation supplied the launcher correction.
- Final implementation report `task-4-prescribed-working-scale-implementation-report.md`, SHA-256 `a39255d3ef4d46995d7043a9f31343fa9f49eb278fd069c957d004a14b1532a6`, MPI report `task-4-prescribed-working-scale-mpi-report.md`, SHA-256 `2f349ca44df76c8b6770fe352c1a340a7e19bf512f3a9589cbc63a8f8513ba7f`, and scoped review `task-4-prescribed-working-scale-review-report.md`, SHA-256 `890b02249cdd578a3b8f3d41afdb38814b782a09a371f0bd933c4080eb603a16`, are retained in the same scratch coordination directory. Integration followed all affected guard releases and preserves the protected diagnostic. Dependency rebuilding dominates the Application lane; provenance checks and wrapper overhead account for most of the 44-second adjacent-test phase despite its 0.08-second test time. No queue delay was observed.
- This is bounded repair and regression evidence, not full prescribed-angle closure. Mixed/high-order target paths, component/range extremes, wider MPI coverage, stage/cadence, curved strips, vanishing maintenance work and authoritative production binding remain incomplete. The next critical-path work is the smallest joint geometry/scalar/derivative/kinematic repair that makes the captured production case pass unchanged gates; rejecting that case is insufficient. WP-4, Task 4, FSR-03, FSR-04 and Q2 remain open, and no audit checkbox or qualification gate changes.

WP-4 authoritative derivative binding prerequisite accepted on 2026-09-06:

- [x] Add borrowed snapshot/functional/source binding with fresh producer-solution sampling through the actual scalar P1 field map, source/layout/parameter checks and collective failure agreement. Valid inputs return `source_branch_unverified` with empty arrays and no success stamps until the shared geometry and retained-support contract is implemented. Legacy numerical overloads are preserved.
- Evidence `/scratch/users/zsexton/wp4-continuation-20260906/binding` binds patch `d60c25d43c11402a724dc664af6024149d54da4e788c24171fa1bf9c78ad9db1`. Job `42262918` passed ten serial cases and the collective-failure case independently on both MPI ranks, with zero nonpass cases and separate XML files. The permutation fixture checks actual mapping `[2,1,0]`, required versus unrelated field values and the legacy derivative-order control. Earlier fixture-numbering and XML-listener failures remain preserved. Job `42263322` separately verified CMake target and serial/two-rank test registration without building a project target or running numerical tests.
- Independent review `binding/review.md`, SHA-256 `bb1ac525cb5b49ddca30d737eaf7dfed086cdce543c9215a9a546d2ab9d91b49`, accepts this prerequisite without blocking findings. It verifies guarded reuse of the unchanged projection object and its 437 dependencies, while recording that 49 external test headers have postcompile checks rather than precompile hashes. Successful bound derivatives, physical work, Application adoption and the captured evaluator/minimizer milestone remain unverified. This API completion does not change a physical gate or WP-4/FSR/Q2 closure status.

WP-4 lifecycle coefficient-policy prerequisite accepted on 2026-09-06:

- [x] Propagate the explicit coefficient-classification policy through generated lifecycle requests, full-cell classification and both cache-signature routes. The default legacy behavior and policy keys are preserved; invalid or unsupported exact-zero selections are rejected before lifecycle mutation.
- The three-file patch `5d4f73a0666db42c37dc5646f344b83d7c16b7477bcfb5668e3575b10eee4305` is recorded at `/scratch/users/zsexton/wp4-continuation-20260906/lifecycle/source.diff`. Runtime RED executed 20 tests with ten intended failures and ten passing controls. GREEN passed the same 20 tests, including Triangle3/Tetra4 cuts at coefficient scales `1e-14`, `1` and `1e14`, policy transitions, reuse and rejection checks, with zero skips. Jobs `42261827` and `42262345` completed on `amarsden`; both input guards released unchanged. GREEN XML SHA-256 is `ce2fa276bd039dcc43a581797a4636fe16e7eee6f176499e3bb920162d793a77`.
- Independent specification and quality review accepts this bounded prerequisite without blocking findings; report `/scratch/users/zsexton/wp4-continuation-20260906/lifecycle/review.md` has SHA-256 `f4547da71ce2be1438d5bb773d417bf541e0e1941d530e4ca59ba0a2f8765704`. The changed implementation compiled in 12.38 seconds; the test command took 0.27 seconds. Six unchanged objects were reused only after checking their products, actual dependencies and compile flags.
- [ ] Complete production adoption, including restart/volume adapters, positive-piece retention, common original-coefficient geometry and derivatives, and rebuilt Application/Physics consumers. The existing restart record would lose an opt-in selection; this must be repaired before relying on persisted production state. The captured evaluator and minimizer must still pass their unchanged gates. WP-4, Task 4, FSR-03, FSR-04 and Q2 remain open.
...
- 2026-09-05 (UTC), architecture coordination: checkpoint 29148f0e is pushed and the implementation source tree is clean. R2 preparation currently writes only private reference/test overlays and Python evidence helpers; no live FE selector or Physics migration is present. The seven-region scalar reference will exercise existing FE installation on full, selected cut, reversed-side, second-marker, retained-support, wet and dry domains. Source review passes; actual baseline execution and numerical acceptance remain pending. Preserve concurrent WP-4 ownership. The next authorized implementation slice is one thin FE volume-selection header plus existing FE test owners, released only after the direct reference is accepted.
...
Ruling: Preserve the exact raw generated-rule global parent sentinel -1 in the bounded scalar verifier, while requiring local parent0 and top-level physical mesh cell_global_id0 separately. Immutable LevelSetCellCutInput carries no global ID; the builder assigns only the local parent and the region default INVALID_GLOBAL_INDEX=-1 is copied unchanged into rule provenance. This fixture directly publishes that domain without mesh rebinding. The previous verifier conflated raw rule provenance with physical mesh identity. Require the precise observed source-backed value, not either ID; preserve all numeric, geometry, side/marker and currentness checks. This local reference establishes no distributed global-parent qualification. If the source interpretation is wrong, acceptance must be withdrawn and re-reviewed.

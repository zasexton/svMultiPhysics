...
- **Next shared-file extraction:** `Application/Core/ApplicationDriver.cpp` will consume the accepted typed maintenance configuration at its two existing setup points, separate mutable runtime and declaration bindings, and remove duplicate maintenance parsing. Full version-4 schedules, trial-copy/rollback state, setup-error order and the frozen lifecycle payload contract are the comparison gates. FE/Physics algorithms, force definitions and numerical values remain outside this driver slice.

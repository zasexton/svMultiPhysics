...
- **Current work:** R0 accepted serial/two-rank wet-block and six one-rank lifecycle references, with broader capture still pending. R1 now consumes immutable typed maintenance configuration in the driver, separates mutable workflow state and declaration bindings, and removes duplicate parsing. Source review and all 368 Application cases plus selected two/four-rank checks passed with no skips; 52 schedule records match the before-data exactly. All six lifecycle payloads match the frozen baseline exactly under the recorded one-rank policy and repeat byte-for-byte. Existing input-path compatibility differences and numerical values are retained. Cut-option consolidation, canonical input reconciliation, physical input adapters and full effective-policy provenance remain open.
...
- **Compute ownership:** Only Slurm job IDs entered at submission in [the refactoring job ledger](free_surface_refactor_job_ledger_20260904.md) may be modified by this work. The ceiling remains four nodes and 80 GB aggregate requested memory on `amarsden`, including pending reservations. The remaining reservation is successor verification allocation `42130677`: one node / 32,000 MiB. Allocation `42093152` completed normally after all of its numeric steps became terminal. Enabled-feature baseline `42090895` completed its 20 declared groups without failures; exclusions and individual skips remain recorded. JIT profile `42080271`, resolver candidate `42083671` and corrected serial/MPI baseline `42086668` completed successfully. Earlier failed configure/build/launch attempts remain recorded; skips and incomplete operator/history captures are not treated as qualification passes. Current state and evidence are in the ledger.

- **Next shared-file extraction:** `Application/Core/ApplicationDriver.cpp` and `Application/Core/LevelSetCutConfiguration.h/.cpp` will share conversion into the existing FE generated-interface and volume option types. First capture the actual old active-cut and maintenance values, omitted/explicit quadrature distinctions and FE failure staging. Temporary guarded observations will be removed after comparison. Public request layout, FE validation timing, physical admission, geometry/force formulas and numerical values remain unchanged during this conversion slice.

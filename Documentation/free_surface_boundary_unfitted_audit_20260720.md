...
- **Current work:** R0 baseline execution and reference capture; first R1 Physics option extraction. The existing free-surface option declarations are moving from `IncompressibleNavierStokesVMSModule.h` to `Physics/Formulations/NavierStokes/FreeSurface/FreeSurfaceOptions.h`, with aliases preserving existing nested C++ names. Aggregation guards reuse `FE::constraints::SmallCutAggregationGuardOptions`; defaults, boundary locality, physical validation and assembly formulas remain unchanged. This slice awaits candidate compilation and behavioral checks.
...
- **Compute ownership:** Only Slurm job IDs entered at submission in [the refactoring job ledger](free_surface_refactor_job_ledger_20260904.md) may be modified by this work. The allocation ceiling is four nodes and 80 GB aggregate requested memory on `amarsden`, including pending reservations. Current reservations are `42080271` (JIT baseline with corrected include environment) and `42080275` (frozen R1 option candidate), totaling two nodes and 38,000 MiB. Core `42068077` completed with retained test-setup failures and 491 passing Physics cases; failed JIT attempts `42068499` and `42070565` remain recorded. Current state and evidence are in the ledger.

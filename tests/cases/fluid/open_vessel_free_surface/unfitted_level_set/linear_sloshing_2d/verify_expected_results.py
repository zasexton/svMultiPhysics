#!/usr/bin/env python3
"""Verify the 2D linear sloshing free-surface case."""
...
def edge_zero_crossings(points: np.ndarray, phi: np.ndarray, cells: np.ndarray) -> list[InterfaceCrossing]:
    crossings: list[InterfaceCrossing] = []
    keys: set[tuple[int, int]] = set()

    def add(a: int, b: int, t: float) -> None:
        p = (1.0 - t) * points[a, :2] + t * points[b, :2]
        key = tuple(np.round(p / DEDUP_TOL).astype(np.int64))
        if key in keys:
            return
        keys.add(key)
        crossings.append(InterfaceCrossing(point=p, edge=(a, b), t=float(t)))
...
def interpolate_crossing_values(values: np.ndarray, crossings: list[InterfaceCrossing]) -> np.ndarray:
    ...
    for crossing in crossings:
        ...
        va = values[a]
        vb = values[b]
        out.append((1.0 - crossing.t) * va + crossing.t * vb)
...
def nonfinite_crossing_endpoint_count(values: np.ndarray, crossings: list[InterfaceCrossing]) -> int:
    ...
    for crossing in crossings:
        a, b = crossing.edge
        if not (np.all(np.isfinite(values[a])) and np.all(np.isfinite(values[b]))):
            count += 1
    return count
...
def clipped_area_and_centroid(points: np.ndarray, phi: np.ndarray, cells: np.ndarray) -> tuple[float, np.ndarray]:
    total_area = 0.0
...
def exact_height(x: np.ndarray, t: float, expected: dict) -> np.ndarray:
    sol = expected["analytic_solution"]
    return sol["H0"] + sol["amplitude"] * np.cos(sol["k"] * x) * math.cos(sol["omega"] * t)
...
def exact_velocity(points: np.ndarray, t: float, expected: dict) -> np.ndarray:
    ...
    x = points[:, 0]
    y = points[:, 1]
    denom = math.sinh(sol["k"] * sol["H0"])
    sinwt = math.sin(sol["omega"] * t)
    out = np.zeros((points.shape[0], 3), dtype=float)
    out[:, 0] = sol["amplitude"] * sol["omega"] * np.cosh(sol["k"] * y) / denom * np.sin(sol["k"] * x) * sinwt
    out[:, 1] = -sol["amplitude"] * sol["omega"] * np.sinh(sol["k"] * y) / denom * np.cos(sol["k"] * x) * sinwt
    return out
...
def exact_pressure(points: np.ndarray, t: float, expected: dict) -> np.ndarray:
    ...
    y = points[:, 1]
    denom = math.sinh(sol["k"] * sol["H0"])
    hydrostatic = fluid["density"] * fluid["gravity"] * (sol["H0"] - y)
    dynamic = (
        fluid["density"]
        * (sol["amplitude"] * sol["omega"] * sol["omega"] / sol["k"])
        * np.cosh(sol["k"] * y)
        / denom
        * np.cos(sol["k"] * x)
        * math.cos(sol["omega"] * t)
    )
...
def compute_metrics(result_path: Path, time: float, expected: dict) -> dict[str, object]:
    ...
    phase = math.atan2(-c_sin, c_cos)
    expected_cos = sol["amplitude"] * math.cos(sol["omega"] * time)
    expected_sin = 0.0
    ...
    velocity_scale = max(rms(vel_exact_norm), sol["amplitude"] * sol["omega"], 1.0e-14)
    ...
    pressure_scale = max(rms(p_exact[wet]), expected["fluid"]["density"] * expected["fluid"]["gravity"] * sol["H0"], 1.0)
    ...
    metrics: dict[str, object] = {
        ...
        "relative_area_error": abs(area - area_expected) / area_expected,
        ...
        "velocity_max_norm": float(np.nanmax(np.linalg.norm(velocity[wet, :2], axis=1))),
        "pressure_rms_error_absolute_gauge": rms(p_error),
    ...
    checks = {
        ...
        "area_relative": metrics["relative_area_error"] <= tolerances["area_relative"],
        "velocity_relative_l2": metrics["velocity_relative_l2_error"] <= tolerances["velocity_relative_l2"],
        "velocity_max_abs": metrics["velocity_max_abs_error"] <= tolerances["velocity_max_abs"],

...
DISCOVERY_HELPER_PATH = SCRIPT_PATH.with_name(
    "mpi_aware_gtest_discovery.py"
)
EXPECTED_PARENT_RUNNER_SHA256 = (
    "640a2cd920fce4309e5d625d2c9c5345ebffd741bb0a0dcd128cc73f0ebd37f7"
)
...
def _load_parent_runner() -> Any:
    ...
    specification = importlib.util.spec_from_file_location(
        "_free_surface_wp3_sharp_boundary_v4_parent",
        PARENT_RUNNER_PATH,
    )
...
def _load_discovery_helper() -> Any:
    if hashlib.sha256(DISCOVERY_HELPER_PATH.read_bytes()).hexdigest() != (
        EXPECTED_DISCOVERY_HELPER_SHA256
    ):
        raise RuntimeError("test discovery helper bytes changed")
    specification = importlib.util.spec_from_file_location(
        "_free_surface_wp3_sharp_boundary_v4_discovery",
        DISCOVERY_HELPER_PATH,
    )
    if specification is None or specification.loader is None:
        raise RuntimeError("cannot load the test discovery helper")
    module = importlib.util.module_from_spec(specification)
    sys.modules[specification.name] = module
    specification.loader.exec_module(module)
    return module
...
_discovery = _load_discovery_helper()
...
_shared_untracked_source_record = _parent._shared_untracked_source_record
_shared_run_build_phase = _parent._shared_run_build_phase

EXPECTED_MATRIX_ID = "free_surface_wp3_sharp_boundary_closure_v4"
...
EXPECTED_IMPLEMENTATION_SOURCE_COMMIT = (
    "5fa2bd550c6b3eb89eaaba517e7f051178487f45"
)
EXPECTED_NORMALIZED_REGISTRY_SHA256 = (
    "d1307367a3deae5d0c1353adc68812022d0549beaf837bbf304c80df60d936c4"
)
...
EXPECTED_STATUS_REASONS = {
    DRAFT_MATRIX_STATUS: (
        ...
        "finalized. No V4 qualification evidence has been executed."
    ),
    EXECUTABLE_MATRIX_STATUS: (
        ...
        "bundle commit. No V4 qualification evidence has been executed."
    ),
}
...
EXPECTED_IMPLEMENTATION_SOURCES_SHA256 = (
    "cd9289e8d44040ef891e6447d4e0991b2c0a996d6f260d74a7fee92bb6bbdbbf"
)
EXPECTED_PARENT_ARTIFACTS_SHA256 = (
    "b5d55754ce790cea7c80a418cc87ecf16ae5bf165b53c58a6ea6d3f7e3d045c4"
)
...
EXPECTED_RESOURCE_SAFEGUARDS_SHA256 = (
    "c6653a41013523654e91e0d1f8e125b2131798a65151a54cc182d7c685256139"
)
EXPECTED_TEST_DISCOVERY_CONTRACT_SHA256 = (
    "2c7ba692b8047854183be86f4cc65c0802b326a016c811c76a9a2c3404484459"
)
EXPECTED_PARENT_ARTIFACT_HASHES = {
    (
        "tests/cases/fluid/"
        "free_surface_wp3_sharp_boundary_qualification_matrix_v3.json"
    ): "5d522f06159d8f2c5e1762794eaa0f43ac6126ba149df0f7b8517ebd69505d02",
    (
        "tests/cases/fluid/"
        "run_free_surface_wp3_sharp_boundary_qualification_v3.py"
    ): EXPECTED_PARENT_RUNNER_SHA256,
...
def validate_matrix_contract(registry: dict[str, Any]) -> dict[str, Any]:
    ...
    discovery_contract = registry.get("test_discovery_contract")
    if (
        discovery_contract != EXPECTED_TEST_DISCOVERY_CONTRACT
        or _canonical_sha256(discovery_contract)
        != EXPECTED_TEST_DISCOVERY_CONTRACT_SHA256
    ):
        raise ValueError("test discovery contract changed")
    ...
    mpi_binary_keys = _discovery.mpi_initializing_binary_keys(
        groups,
        EXPECTED_BINARY_KEYS,
    )
    if mpi_binary_keys != set(
        EXPECTED_TEST_DISCOVERY_CONTRACT["mpi_single_rank_binary_keys"]
    ):
        raise ValueError("MPI test discovery routes changed")
    if EXPECTED_BINARY_KEYS - mpi_binary_keys != set(
        EXPECTED_TEST_DISCOVERY_CONTRACT["direct_binary_keys"]
    ):
        raise ValueError("direct test discovery routes changed")
...
def load_registry(path: Path) -> dict[str, Any]:
    ...
    if path.resolve() != DEFAULT_REGISTRY.resolve():
        raise ValueError("qualification requires the canonical V4 matrix")
    if path.is_symlink() or not path.is_file():
        raise ValueError("canonical V4 matrix must be a regular file")
...
def _inject_provenance(value: dict[str, Any]) -> None:
    ...
    value["test_discovery_contract"] = copy.deepcopy(
        EXPECTED_TEST_DISCOVERY_CONTRACT
    )
...
def validate_only_summary(
    ...
    return {
        ...
        "test_discovery_contract": copy.deepcopy(
            EXPECTED_TEST_DISCOVERY_CONTRACT
        ),
        "closure_outcome": (
...
def create_test_discovery(
    ...
    contract = registry["test_discovery_contract"]
    discovery = _discovery.MPIAwareGTestDiscovery(
        binaries,
        registry["groups"],
        mpi_launcher,
        timeout_seconds=contract["timeout_seconds"],
    )
    if discovery.contract() != contract:
        raise ValueError("test discovery implementation differs from contract")
    return discovery
...
def main(arguments: list[str] | None = None) -> int:
    ...
    test_discovery = create_test_discovery(
        binaries,
        registry,
        execution.mpiexec,
    )

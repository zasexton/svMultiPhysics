...
class BoundarySegment:
    """One extracted boundary segment (as line cells) with parent-element mapping."""
    name: str
...
def extract_boundary_segments_by_crease_angle_with_parent_ids(
    ...
    *,
    ...
    """
    Given a planar 2D mesh (VTU / UnstructuredGrid with triangle/quad cells),
    ...
    For every LINE cell in each output PolyData:
      - cell_data['GlobalElementID'] is set to the parent 2D element cell ID (0-based)
        from the input mesh that owns that boundary edge (triangle or quad).
    ...
    def build_segment_polydata(edges, edge_indices, *, pts, edge_owner, clean_tol=1e-12):
        ...
        and attach GlobalElementID per line cell mapping to parent element id.
        """
        used_pids = set()
        seg_edges = []
        seg_parent = []
        ...
        for ei in edge_indices:
            ...
            used_pids.add(e[0])
            used_pids.add(e[1])
            seg_parent.append(int(edge_owner[e]))  # parent element id in original mesh

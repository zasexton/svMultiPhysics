...
def _load_audit_module():
    ...
    script = (
        ...
        / "audit_direct_pspg_cut_volume_gradient_column_graph_selectivity.py"
    )
    spec = importlib.util.spec_from_file_location(
        "audit_direct_pspg_cut_volume_gradient_column_graph_selectivity",
        script,
    )
...
def _entry(op, rule_index, row_dof, mismatch=False):
    sampled_grams = "5|-2|-2|-1"
    if mismatch:
        sampled_grams = "5|2|-2|-1"
    return (
        "StandardAssembler: "
        ...
        "row_abs_sum=10 row_signed_sum=0 positive_sum=5 "
...
def test_gradient_column_graph_audit_uses_latest_rule_index_reset_batch(tmp_path):
    ...
    log.write_text(
        "\n".join(
            [
                _entry(op, 10, 100, mismatch=False),
                _entry(op, 11, 101, mismatch=False),
                _entry(op, 1, 100, mismatch=True),
                _entry(op, 2, 101, mismatch=False),
            ]
        ),
        encoding="utf-8",
    )
            ...
            "cases": [
                {
                    "label": "test02",
                    "path": str(
                        log.with_name("run_direct_pspg_cut_volume_row_provenance.log")
                    ),
    ...
    target = case["profile_summary"]["target_profiles"]["100"]
    assert target["matrix_gradient_sign_mismatch_count"] == 1
    selectors = {selector["key"]: selector for selector in report["selectors"]}
    sign_mismatch = selectors["gradient_column_graph_sign_mismatch"]["cases"][0]
    assert sign_mismatch["finding"] == "selector_selective"
    assert sign_mismatch["covered_direct_target_global_dofs"] == [100]


def test_gradient_column_graph_audit_flags_missing_logs():
    audit = _load_audit_module()
    report = audit.build_report(
        global_emission={
            "cases": [
    ...
    assert (
        report["finding"]
        == "direct_pspg_cut_volume_gradient_column_graph_evidence_missing"
    )
    assert report["missing_case_labels"] == ["test02"]


def test_gradient_column_graph_audit_cli_style_report_paths(tmp_path):
    audit = _load_audit_module()

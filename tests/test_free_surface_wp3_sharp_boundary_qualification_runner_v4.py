#!/usr/bin/env python3
"""Focused contract tests for the WP-3 sharp-boundary V4 runner."""
...
def _load_runner():
    specification = importlib.util.spec_from_file_location(
        "_focused_wp3_sharp_boundary_v4",
        RUNNER_PATH,
    )
...
class SharpBoundaryV4ContractTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_matrix_contract_has_complete_frozen_inventory(self) -> None:
        ...
        self.assertEqual(len(validated["implementation_sources"]), 45)
        self.assertEqual(
            validated["test_discovery_contract"],
    ...
    def test_group_and_evidence_digests_are_independent_locks(self) -> None:
        ...
        self.assertEqual(
            runner._canonical_sha256(self.registry["parent_artifacts"]),
            runner.EXPECTED_PARENT_ARTIFACTS_SHA256,
        )
        self.assertEqual(
            runner._canonical_sha256(
                self.registry["resource_safeguards"]
            ),
            runner.EXPECTED_RESOURCE_SAFEGUARDS_SHA256,
        )
        self.assertEqual(
            runner._canonical_sha256(
                self.registry["test_discovery_contract"]
            ),
            runner.EXPECTED_TEST_DISCOVERY_CONTRACT_SHA256,
        )
    ...
    def test_source_manifest_matches_recorded_commit(self) -> None:
        ...
        self.assertEqual(
            runner.sha256_file(runner.DISCOVERY_HELPER_PATH),
            runner.EXPECTED_DISCOVERY_HELPER_SHA256,
        )

    def test_discovery_contract_matches_groups_and_helper(self) -> None:
        binaries = {
            key: Path(f"/scratch/tests/{key}")
    ...
    def test_discovery_contract_mutation_is_rejected(self) -> None:
        changed = copy.deepcopy(self.registry)
        changed["test_discovery_contract"]["timeout_seconds"] = 61
        with self.assertRaisesRegex(ValueError, "discovery contract changed"):
            runner.validate_matrix_contract(changed)

    def test_discovery_contract_is_injected_into_evidence(self) -> None:
        with mock.patch.object(runner, "_shared_write_json") as write:
            runner.write_json(
                Path("/scratch/tests/build_preflight.json"),
                {"phase": "preflight"},
            )
        written = write.call_args.args[1]
        self.assertEqual(
            written["test_discovery_contract"],
            runner.EXPECTED_TEST_DISCOVERY_CONTRACT,
        )
        self.assertEqual(
            written["implementation_source_commit"],
    ...
    def test_scheduler_allocation_contract_rejects_resource_drift(self) -> None:
        ...
        self.assertEqual(observed["tasks"], 10)
    ...
    def test_validate_only_summary_keeps_later_work_open(self) -> None:
        ...
        self.assertEqual(
            summary["test_discovery_contract"],
            runner.EXPECTED_TEST_DISCOVERY_CONTRACT,
        )
